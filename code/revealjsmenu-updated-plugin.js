  ...
  function initMenu() {
    if (!initialised) {
      ...
      function generateItem(type, section, i, h, v) {
        ...
        if (options.markers) {
          let sur = create('div', {class: 'treeicons', style: 'display: inline; margin-right: 5px;'});
          sur.appendChild(create('i', { class: 'fas fa-plus-circle fa-fw past fa-lg' })); // past = closed
          sur.appendChild(create('i', { class: 'fas fa-minus-circle fa-fw active fa-lg' })); // active = open
          sur.appendChild(create('i', { class: 'far fa-circle fa-fw future fa-lg' })); // future = no children
          sur.addEventListener("click", (n) => {
            sur.parentElement.classList.toggle("unfolded");
            sur.parentElement.classList.toggle("folded");
            n.stopPropagation();
          });
          item.appendChild(sur);
        }
